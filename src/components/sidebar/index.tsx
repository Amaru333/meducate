...
function Sidebar() {
  ...
  return (
    <div className="w-12 lg:w-64 h-full flex flex-col">
      <Link href="/dashboard/home" className="p-2 xl:p-6 border-b h-20 flex items-center justify-center">
        <Image alt="logo" src="/images/logo-small.png" width={50} height={0} sizes="100vw" className="object-contain rounded-2xl" />
      </Link>
      <div className="py-8 px-2 lg:px-6 gap-4 flex flex-col flex-1">
        {navbarData.map((item, idx) => (
          <Link
            href={item.link}
            key={idx}
            className={`flex relative items-center gap-3 px-2 lg:px-4 py-3 rounded-full active:scale-95 hover:bg-primary hover:bg-opacity-5 transition-all overflow-hidden ${pathname?.includes(item.link) ? "bg-primary bg-opacity-10 text-primary" : "bg-transparent text-black"}`}
          >
      ...
      <div className="px-2 lg:px-6 pb-12">
