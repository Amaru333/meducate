...
function CoursesPage() {
  ...
  return (
    <div className="flex flex-col mb-6">
      ...
      <div className="flex flex-row gap-x-8 overflow-x-scroll flex-1 w-[calc(100vw-20px)] lg:w-[calc(100vw-256px)] no-scrollbar pb-2">
        <div className="w-8 h-full"></div>
        {myCourses.map((course, i) => (
          <DefaultCourseCard data={course} className="w-full min-w-80 max-w-80" key={i} />
        ))}
        <div className="w-8 h-full"></div>
      </div>
